...
///MediaUI: Presents the attached media picker. To get the picked item, use MediaPicker's projected value ($picker)
    @MainActor public func presentPicker() throws {
        Task {
            try await MediaPickerData.shared.present(picker: .single)
        }
    }
///MediaUI: Presents the attached media picker then returns the picked item.
